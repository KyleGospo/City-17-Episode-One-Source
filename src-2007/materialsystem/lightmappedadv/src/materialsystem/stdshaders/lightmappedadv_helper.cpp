...
static ConVar mat_disable_lightwarp( "mat_disable_lightwarp", "0" );
static ConVar mat_disable_fancy_blending( "mat_disable_fancy_blending", "0" );
static ConVar mat_fullbright( "mat_fullbright", "0", FCVAR_CHEAT );

ConVar mat_force_lightmapped_phong( "mat_force_lightmapped_phong", "0", FCVAR_CHEAT );
...
void InitParamsLightmappedAdv_DX9( CBaseVSShader *pShader, IMaterialVar** params, const char *pMaterialName, LightmappedAdv_DX9_Vars_t &info )
{
	...
	else if ( mat_force_lightmapped_phong.GetBool() &&
...
void DrawLightmappedAdvFlashlight_DX9_Internal( CBaseVSShader *pShader, IMaterialVar** params, IShaderDynamicAPI *pShaderAPI, 
												IShaderShadow* pShaderShadow, LightmappedAdvFlashlight_DX9_Vars_t &vars )
{
...
void DrawLightmappedAdv_DX9_Internal( CBaseVSShader *pShader, IMaterialVar** params, bool hasFlashlight, 
		...
		if ( hasBump && g_pHardwareConfig->SupportsNormalMapCompression() && g_pHardwareConfig->SupportsPixelShaders_2_b() )
		{
			...
			if ( pBumpTex )
			{
				...
				if ( hasBump2 )			// Check encoding of secondary normal if there is oneg
				{
					...
					{
						DevMsg("LightmappedAdv: Primary and Secondary normal map compression formats don't match.  This is unsupported!\n");
						Assert(0);
					}
				}
			}
							 ...
							 LightmappedAdv_DX9_Vars_t &info, CBasePerMaterialContextData **pContextDataPtr )
{
	...
	DrawLightmappedAdv_DX9_Internal( pShader, params, hasFlashlight, pShaderAPI, pShaderShadow, info, pContextDataPtr );
}
