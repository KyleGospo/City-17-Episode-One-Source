...
void DrawWater( in DrawWater_params_t i, 
				...
				in sampler NormalSampler2,
				in sampler FlowMapSampler,
				in sampler NoiseMapSampler,
...
#else
	#if FLOWMAP
		float2 flowmap = tex2D( FlowMapSampler, i.vBumpTexCoord ).rg * 2.0f - 1.0f;
		float cycleOffset = tex2D( NoiseMapSampler, i.vBumpTexCoord ).r;

		float phase0 = cycleOffset * .5f + FlowMapOffset0;
		float phase1 = cycleOffset * .5f + FlowMapOffset1;

		// Sample normal map.
		//float3 normalT0 = tex2D(NormalSampler, ( i.vBumpTexCoord * i.scale.x ) + flowmap * phase0 );
		//float3 normalT1 = tex2D(NormalSampler2, ( i.vBumpTexCoord * i.scale.y ) + flowmap * phase1 );
		float4 normalT0 = DecompressNormal( NormalSampler, (i.vBumpTexCoord * i.scale) + flowmap * phase0, NORMAL_DECODE_MODE );
		float4 normalT1 = DecompressNormal( NormalSampler2, (i.vBumpTexCoord * i.scale) + flowmap * phase0, NORMAL_DECODE_MODE );

		float flowLerp = ( abs( HalfCycle - FlowMapOffset0 ) / HalfCycle );
		float3 offset = lerp( normalT0.rgb, normalT1.rgb, flowLerp );
		float4 vNormal = float4( offset, normalT0.a );
	#else
...
#	endif
	...
	float offset = i.focusoffset/1000;
	vRefractColor.r = tex2D(RefractSampler, vRefractTexCoord+float2(offset,0)).r;
	vRefractColor.g = tex2D(RefractSampler, vRefractTexCoord-float2(offset,0)).g;
	vRefractColor.b = tex2D(RefractSampler, vRefractTexCoord-float2(offset,0)).b;
#endif
