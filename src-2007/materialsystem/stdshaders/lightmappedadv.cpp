...
#include "lightmappedadv_helper.h"

static LightmappedAdv_DX9_Vars_t s_info;
static LightmappedAdv_DX9_Vars_t s_info_editor;


DEFINE_FALLBACK_SHADER( LightmappedAdv, LightmappedAdv_DX9 )

BEGIN_VS_SHADER( LightmappedAdv_DX9, "Shader for lightmapped world geometry" )
	...
	void SetupVars( LightmappedAdv_DX9_Vars_t &info )
	{
	...
	FORCEINLINE void SwapLayers( LightmappedAdv_DX9_Vars_t &info )
	{
		swap( info.m_nBaseTexture,			info.m_nBaseTexture2 );
		swap( info.m_nBaseTextureFrame,		info.m_nBaseTexture2Frame );
		swap( info.m_nBaseTextureNoEnvmap,	info.m_nBaseTexture2NoEnvmap );
		swap( info.m_nBumpmap,				info.m_nBumpmap2 );
		swap( info.m_nBumpFrame,			info.m_nBumpFrame2 );
		swap( info.m_nBumpTransform,		info.m_nBumpTransform2 );
	}

	// Is material WorldVertexTransition?
	FORCEINLINE bool IsWVT( IMaterialVar **params ) const
	{
		return params[BASETEXTURE2]->IsDefined() && params[BASETEXTURE2]->IsTexture();
	}

	SHADER_FALLBACK
	{
		if ( g_pHardwareConfig->GetDXSupportLevel() < 90 )
			return IsWVT( params ) ? "WorldVertexTransition_DX8" : "LightmappedGeneric_DX8";
	...
	{
		SetupVars( s_info );
		InitParamsLightmappedAdv_DX9( this, params, pMaterialName, s_info );
	...
	{
		SetupVars( s_info );
		InitLightmappedAdv_DX9( this, params, s_info );
	}

	SHADER_DRAW
	{
		if ( IsWVT( params ) && UsingEditor( params ) )
			DrawLightmappedAdv_DX9( this, params, pShaderAPI, pShaderShadow, s_info_editor, pContextDataPtr );
		else
			DrawLightmappedAdv_DX9( this, params, pShaderAPI, pShaderShadow, s_info, pContextDataPtr );
	}
END_SHADER
