...
#include "radialgaussiany_ps20.inc"
#include "radialgaussiany_ps20b.inc"

// memdbgon must be the last include file in a .cpp file!!!
#include "tier0/memdbgon.h"

BEGIN_VS_SHADER( RadialGaussianY, "Help for Radial Gaussian Y" )
	BEGIN_SHADER_PARAMS
	...
	SHADER_DRAW
	{
		SHADOW_STATE
		{
			...
			{
				DECLARE_STATIC_PIXEL_SHADER( radialgaussiany_ps20b );
				SET_STATIC_PIXEL_SHADER( radialgaussiany_ps20b );
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER( radialgaussiany_ps20 );
				SET_STATIC_PIXEL_SHADER( radialgaussiany_ps20 );
			}
		}
		...
		DYNAMIC_STATE
		{
			...
			{
				fResolution[0] = float(nHeight);
			}
			else
			{
				fResolution[0] = float(nHeight/params[RESDIVISOR]->GetIntValue());
			}
			...
			if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( radialgaussiany_ps20b );
				SET_DYNAMIC_PIXEL_SHADER( radialgaussiany_ps20b );
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( radialgaussiany_ps20 );
				SET_DYNAMIC_PIXEL_SHADER( radialgaussiany_ps20 );
			}
		}
		Draw();
	}
