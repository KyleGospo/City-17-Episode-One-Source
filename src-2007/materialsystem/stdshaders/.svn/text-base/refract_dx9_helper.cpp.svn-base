...
void InitParamsRefract_DX9( CBaseVSShader *pShader, IMaterialVar** params, const char *pMaterialName, Refract_DX9_Vars_t &info )
{
	...
	if( !params[info.m_nChromatic]->IsDefined() )
	{
		params[info.m_nChromatic]->SetIntValue( 0 );
	}
	if( !params[info.m_nChromaticScale]->IsDefined() )
	{
		params[info.m_nChromaticScale]->SetFloatValue( 1.0 );
	}
					 ...
					 IShaderShadow* pShaderShadow, Refract_DX9_Vars_t &info, VertexCompressionType_t vertexCompression )
{
	...
	bool bChromatic = (params[info.m_nChromatic]->GetIntValue() != 0);
	...
	DYNAMIC_STATE
	{
		...
		fFocusOffset[0] = params[info.m_nChromaticScale]->GetFloatValue();
		fFocusOffset[1] = fFocusOffset[2] = fFocusOffset[3] = fFocusOffset[0];
		pShaderAPI->SetPixelShaderConstant( 6, fFocusOffset );
