...
void CBaseHLCombatWeapon::ItemHolsterFrame( void )
{
	...
	if ( ( gpGlobals->curtime - m_flHolsterTime ) > sk_auto_reload_time.GetFloat() )
	{
		...
		//FinishReload(); -- City17: No longer do this.
...
/*static ConVar	cl_bobcycle_max( "v_bobcycle_max", "0.45", FCVAR_CHEAT );
//static ConVar	cl_bob( "cl_bob","0.002" );
static ConVar	cl_bobup( "v_bobup","0.5", FCVAR_CHEAT );*/
...
//			viewmodelindex - 
//-----------------------------------------------------------------------------
static ConVar	v_bob_mull_z( "v_bob_mul_up","0.1", FCVAR_CHEAT );
static ConVar	v_bob_mull_roll( "v_bob_mul_roll","0.5", FCVAR_CHEAT );
static ConVar	v_bob_mull_pitch( "v_bob_mul_pitch","0.4", FCVAR_CHEAT );
static ConVar	v_bob_mull_yaw( "v_bob_mul_yaw","0.3", FCVAR_CHEAT );
static ConVar	v_bob_mull_base( "v_bob_mul_foreward","0.1", FCVAR_CHEAT );
static ConVar	v_bob_mull_final( "v_bob_mul_right","0.8", FCVAR_CHEAT );
void CBaseHLCombatWeapon::AddViewmodelBob( CBaseViewModel *viewmodel, Vector &origin, QAngle &angles )
{
	...
	VectorMA( origin, g_verticalBob * v_bob_mull_base.GetFloat(), forward, origin );
	
	// Z bob a bit more
	origin[2] += g_verticalBob * v_bob_mull_z.GetFloat();
	
	// bob the angles
	angles[ ROLL ]	+= g_verticalBob * v_bob_mull_roll.GetFloat();
	angles[ PITCH ]	-= g_verticalBob * v_bob_mull_pitch.GetFloat();

	angles[ YAW ]	-= g_lateralBob  * v_bob_mull_yaw.GetFloat();

	VectorMA( origin, g_lateralBob * v_bob_mull_final.GetFloat(), right, origin );
}*/
