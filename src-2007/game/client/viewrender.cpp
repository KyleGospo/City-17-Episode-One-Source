...
static ConVar r_threaded_renderables( "r_threaded_renderables", "0", FCVAR_CHEAT );
...
void CViewRender::DetermineWaterRenderInfo( const VisibleFogVolumeInfo_t &fogVolumeInfo, WaterRenderInfo_t &info )
{
	...
	info.m_bDrawWaterSurface = (mat_drawwater.GetBool() /*&& (CurrentViewID() != VIEW_SUN_SHAFTS)*/ );
...
void CRendering3dView::SetupRenderablesList( int viewID )
{
	...
	if( m_pMainView->ShouldDrawEntities() )
	{
		...
		setupInfo.m_bDrawDetailObjects = g_pClientMode->ShouldDrawDetailObjects() && r_DrawDetailProps.GetInt() /*&& viewID != VIEW_SUN_SHAFTS*/;
...
void CRendering3dView::BuildRenderableRenderLists( int viewID )
{
	...
	if ( viewID != VIEW_SHADOW_DEPTH_TEXTURE /*&& viewID != VIEW_SUN_SHAFTS*/ )
	{
...
void CBaseWorldView::DrawExecute( float waterHeight, view_id_t viewID, float waterZAdjust )
{
	...
	{
		/*if( g_CurrentViewID == VIEW_SUN_SHAFTS )
		{
			//City 17: Render translucents BEFORE opaques!
	...
	if ( !IsMainView( CurrentViewID() ) && CurrentViewID() != VIEW_INTRO_CAMERA /*&& CurrentViewID() != VIEW_SUN_SHAFTS*/ )
	{
		PixelVisibility_EndCurrentView();
	}
