...
void CAI_BaseNPC::Event_Killed( const CTakeDamageInfo &info )
{
	...
	/*if( g_MaxsiDistribution != NULL )
	{
		...
		{
			g_MaxsiDistribution->Statistic("Ally NPC Deaths",DeathMap);
		}
		else if( Classify() == CLASS_PLAYER_ALLY_VITAL )
		{
			g_MaxsiDistribution->Statistic("Vital Ally NPC Deaths",DeathMap);
		}
	}*/
