...
#ifdef C17_HAPTICS
// Haptics ( 0 = work by tick interval , 1 = work by time interval ( ms ) )
#define HAP_VEHICLE_TIME_BASE 0
#endif
...
#ifdef C17_HAPTICS
	// Haptics virtual override. handles sending data to client.
	void			VPhysicsUpdate( IPhysicsObject *pPhysics );
#endif
...
protected:
	...
	// Haptics handles rate of vehicle updates to the client of the driver (time).
	float		m_flNextHapticsReport;
#else
	// Haptics handles rate of vehicle updates to the client of the driver (calls).
	int			m_iHapticsSample;
#endif
#endif
};
