...
BEGIN_DATADESC( CEnvProjectedTexture )
	...
	DEFINE_KEYFIELD( m_bAtten, FIELD_BOOLEAN, "useatten" ),
	...
	DEFINE_AUTO_ARRAY_KEYFIELD( m_SpotlightDX8TextureName, FIELD_CHARACTER, "dx8texturename" ),
	DEFINE_KEYFIELD( m_nSpotlightTextureFrame, FIELD_INTEGER, "textureframe" ),
	DEFINE_KEYFIELD( m_flNearZ, FIELD_FLOAT, "nearz" ),
	DEFINE_KEYFIELD( m_flFarZ, FIELD_FLOAT, "farz" ),
	DEFINE_KEYFIELD( m_flAtten, FIELD_FLOAT, "atten" ),
	DEFINE_KEYFIELD( m_flQuadratic, FIELD_FLOAT, "quadraticatten" ),
	...
	DEFINE_INPUTFUNC( FIELD_BOOLEAN, "UseAtten", InputSetUseAtten ),
	...
	DEFINE_INPUTFUNC( FIELD_FLOAT, "Atten", InputSetAtten ),
	DEFINE_INPUTFUNC( FIELD_FLOAT, "QuadraticAtten", InputSetQuadratic ),
	DEFINE_INPUTFUNC( FIELD_STRING, "SpotlightTexture", InputSetSpotlightTexture ),
	DEFINE_INPUTFUNC( FIELD_STRING, "SpotlightDX8Texture", InputSetSpotlightDX8Texture ),
...
IMPLEMENT_SERVERCLASS_ST( CEnvProjectedTexture, DT_EnvProjectedTexture )
	...
	SendPropFloat( SENDINFO( m_fBrightness ) ),
	...
	SendPropString( SENDINFO( m_SpotlightDX8TextureName ) ),
	...
	SendPropFloat( SENDINFO( m_flAtten ) ),
	SendPropFloat( SENDINFO( m_flQuadratic ) ),
...
CEnvProjectedTexture::CEnvProjectedTexture( void )
{
	...
	Q_strcpy( m_SpotlightDX8TextureName.GetForModify(), "effects/flashlight001" );
...
bool CEnvProjectedTexture::KeyValue( const char *szKeyName, const char *szValue )
{
	...
	else if ( FStrEq( szKeyName, "dx8texturename" ) )
	{
#if defined( _X360 )
		if ( Q_strcmp( szValue, "effects/flashlight001" ) == 0 )
		{
			// Use this as the default for Xbox
			Q_strcpy( m_SpotlightDX8TextureName.GetForModify(), "effects/flashlight_border" );
		}
		else
		{
			Q_strcpy( m_SpotlightDX8TextureName.GetForModify(), szValue );
		}
#else
		Q_strcpy( m_SpotlightDX8TextureName.GetForModify(), szValue );
#endif
...
bool CEnvProjectedTexture::GetKeyValue( const char *szKeyName, char *szValue, int iMaxLen )
{
	...
	{
		Q_snprintf( szValue, iMaxLen, "%s", m_SpotlightDX8TextureName.Get() );
		return true;
	}
...
void CEnvProjectedTexture::InputSetUseAtten( inputdata_t &inputdata )
{
	m_bAtten = inputdata.value.Bool();
}
...
{
	m_fBrightness = inputdata.value.Float();
}
...
void CEnvProjectedTexture::InputSetAtten( inputdata_t &inputdata )
{
	m_flAtten = inputdata.value.Float();
}

void CEnvProjectedTexture::InputSetQuadratic( inputdata_t &inputdata )
{
	m_flQuadratic = inputdata.value.Float();
}
...
void CEnvProjectedTexture::InputSetSpotlightDX8Texture( inputdata_t &inputdata )
{
	Q_strcpy( m_SpotlightDX8TextureName.GetForModify(), inputdata.value.String() );
}
