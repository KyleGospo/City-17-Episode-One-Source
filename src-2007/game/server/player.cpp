...
#ifdef C17_HAPTICS
		// Haptics addition		
		QAngle angleOffset = QAngle(random->RandomInt(-0.1,0.1), random->RandomInt(-0.1,0.1), random->RandomInt(-0.1,0.1));
		ViewPunch(angleOffset);
#else
...
int CBasePlayer::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	// Armor. 
	//City17: No armor on our rebel. Don't absorb any damage.
...
void CBasePlayer::Event_Killed( const CTakeDamageInfo &info )
{
	...
	//Maxsi Distribution Death Count
	/*if( g_MaxsiDistribution != NULL )
	{
		char *DeathMap = (char *)STRING(gpGlobals->mapname);
		g_MaxsiDistribution->Statistic("Deaths",DeathMap);
	}*/
