...
void CNPC_Houndeye::Precache()
{
	...
	PrecacheParticleSystem( "citadel_shockwave_d" );
...
void CNPC_Houndeye::SonicAttack ( void )
{
	...
	DispatchParticleEffect( "citadel_shockwave_d", EyePosition(), GetLocalAngles() );
...
void CNPC_Houndeye::StartTask( const Task_t *pTask )
{
	...
	case TASK_HOUND_GET_PATH_TO_CIRCLE:
	{
		...
		else 
		{
			...
			if (GetNavigator()->SetRadialGoal(vTargetPos, random->RandomInt(50,500), 90, 175, m_bLoopClockwise, false))
			{
				TaskComplete();
				return;
			}
