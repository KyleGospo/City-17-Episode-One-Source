...
void CNPC_Combine::PostNPCInit()
{
	if( IsElite() )
	{
		...
		// an AR2. 
		if( !GetActiveWeapon() || !FClassnameIs( GetActiveWeapon(), "weapon_ar2" ) || !FClassnameIs( GetActiveWeapon(), "weapon_ar3" ) )
		{
			DevWarning("**Combine Elite Soldier MUST be equipped with AR2 or AR3\n");
		}
	}
...
void CNPC_Combine::Ignite( float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner )
{
...
{
	if( ( FClassnameIs( pWeapon, "weapon_ar2" ) ) || ( FClassnameIs( pWeapon, "weapon_ar3" ) ) )
	{
