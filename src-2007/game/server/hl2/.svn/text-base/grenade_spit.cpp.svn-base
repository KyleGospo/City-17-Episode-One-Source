...
ConVar	sk_bullsquid_spit_grenade_dmg					( "sk_bullsquid_spit_grenade_dmg", "10", FCVAR_NONE, "Total damage done by an individual bullsquid loogie.");
ConVar	sk_bullsquid_spit_grenade_radius				( "sk_bullsquid_spit_grenade_radius","50", FCVAR_NONE, "Radius of effect for a bullsquid spit grenade.");
ConVar	sk_bullsquid_spit_grenade_poison_ratio			( "sk_bullsquid_spit_grenade_poison_ratio","0.0", FCVAR_NONE, "Percentage of a bullsquid's spit damage done as poison (which regenerates)");

LINK_ENTITY_TO_CLASS( grenade_bullspit, CGrenadeSpit );
...
void CGrenadeSpit::Spawn( void )
{
	...
	if( FClassnameIs( this, "grenade_bullspit" ) )
	{
		m_fIsBullSquid = true;
	}
	else
	{
		m_fIsBullSquid = false;
	}
	...
	if ( m_hSpitEffect != NULL )
	{
		...
		{
			m_hSpitEffect->KeyValue( "effect_name", "bullsquid_spit_trail" );
		}
...
void CGrenadeSpit::GrenadeSpitTouch( CBaseEntity *pOther )
{
	...
	if( m_fIsBullSquid )
	{
		poisonratio = sk_bullsquid_spit_grenade_poison_ratio.GetFloat();
	}
	else
	{
		poisonratio = sk_antlion_worker_spit_grenade_poison_ratio.GetFloat();
	}
	...
	if ( pOther->IsPlayer() || bHitWater )
	{
		...
		{
			DispatchParticleEffect( "bullsquid_spit_player", GetAbsOrigin(), vecAngles );
		}
	}
	else
	{
		...
		{
			DispatchParticleEffect( "bullsquid_spit", GetAbsOrigin(), vecAngles );
		}
	}
...
void CGrenadeSpit::InitHissSound( void )
{
	...
	if ( m_pHissSound == NULL )
	{
		...
		if( m_fIsBullSquid )
		{
			m_pHissSound = controller.SoundCreate( filter, entindex(), "NPC_Bullsquid.Acid" );
		}
		else
		{
...
void CGrenadeSpit::Precache( void )
{
	...
	//Bullsquid
	PrecacheParticleSystem( "bullsquid_spit_player" );
	PrecacheParticleSystem( "bullsquid_spit" );
}
