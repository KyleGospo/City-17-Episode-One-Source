...
ConVar hopwire_strider_kill_dist_h( "hopwire_strider_kill_dist_h", "1024", FCVAR_CHEAT );
ConVar hopwire_strider_kill_dist_v( "hopwire_strider_kill_dist_v", "1024", FCVAR_CHEAT );
ConVar hopwire_strider_hits( "hopwire_strider_hits", "1", FCVAR_CHEAT );
ConVar hopwire_hopheight( "hopwire_hopheight", "700", FCVAR_CHEAT );
ConVar hopwire_blast_duration( "hopwire_blast_duration", "12", FCVAR_CHEAT );
ConVar hopwire_radius( "hopwire_radius", "720", FCVAR_CHEAT );
ConVar hopwire_strength( "hopwire_strength", "300", FCVAR_CHEAT );
ConVar hopwire_shake_amplitude( "hopwire_shake_amplitude", "11.0", FCVAR_CHEAT );
ConVar hopwire_shake_frequency( "hopwire_shake_frequency", "200.0", FCVAR_CHEAT );
ConVar hopwire_shake_duration( "hopwire_shake_duration", "16.0", FCVAR_CHEAT );
ConVar hopwire_shake_radius( "hopwire_shake_radius", "0", FCVAR_CHEAT );

ConVar g_debug_hopwire( "g_debug_hopwire", "0", FCVAR_CHEAT );
...
public:
			...
			CGravityVortexController( void ) : m_flEndTime( 0.0f ), m_flRadius( 720 ), m_flStrength( 300 ), m_flMass( 0.0f ) {}	
...
private:
	...
	void	DissolveEntity( CBaseEntity *pEntity );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CGravityVortexController::DissolveEntity( CBaseEntity *pEntity )
{
	if( pEntity->IsEFlagSet( EFL_NO_DISSOLVE ) )
	{
		//City17: If you're going to be stubborn, just get out.
		UTIL_Remove( pEntity );
		return;
	}
	...
	pEntity->GetBaseAnimating()->Dissolve( "", gpGlobals->curtime, false, ENTITY_DISSOLVE_CORE );
...
void CGravityVortexController::PullPlayersInRange( void )
{
	...
	// If the player doesn't exist, how can we pull him?
	if ( !pPlayer )
		return;
...
bool CGravityVortexController::KillNPCInRange( CBaseEntity *pVictim, IPhysicsObject **pPhysObj )
{
	...
	if ( (pBCC != NULL && pBCC->CanBecomeRagdoll()) && (pBCC->IsNPC()) )
	{
		// NPC check
		if ( ( pBCC->Classify() == CLASS_PLAYER_ALLY ) || ( pBCC->Classify() == CLASS_PLAYER_ALLY_VITAL ) || ( FClassnameIs( pBCC, "npc_strider" ) ) || ( FClassnameIs( pBCC, "npc_combinegunship" ) ) || ( FClassnameIs( pBCC, "npc_combinedropship" ) ) )
			return false;
...
void CGravityVortexController::PullThink( void )
{
	...
	// Kill or stun any striders around us
	KillStriders();
...
#define GRENADE_MODEL	"models/ar3coreball.mdl"
#define GRENADE_MODEL_OPEN "models/ar3_coreball_open.mdl"
...
void CGrenadeHopwire::Precache( void )
{
	...
	//PrecacheParticleSystem( "AR3_child" );
	PrecacheParticleSystem( "AR3_mini" );
	PrecacheParticleSystem( "AR3_coreball" );
...
void CGravityVortexController::KillStriders( void )
{
	CBaseEntity *pEnts[128];
	Vector	mins, maxs;
...
void CGrenadeHopwire::CombatThink( void )
{
	...
	if( pPlayer )
	{
		...
		UTIL_ScreenShake( vec3_origin, hopwire_shake_amplitude.GetFloat(), hopwire_shake_frequency.GetFloat(), hopwire_shake_duration.GetFloat(), hopwire_shake_radius.GetFloat(), SHAKE_START, true );
	}

	// Unleash hell
	DispatchParticleEffect( "AR3_mini", GetAbsOrigin(), vec3_angle );

	m_hVortexController = CGravityVortexController::Create( GetAbsOrigin(), hopwire_radius.GetFloat(), hopwire_strength.GetFloat(), hopwire_blast_duration.GetFloat() );
	...
	SetNextThink( gpGlobals->curtime + hopwire_blast_duration.GetFloat() );
}
...
void CGrenadeHopwire::Detonate( void )
{
	...
	// Inform the NPCs that hell has been unleashed
	CSoundEnt::InsertSound( SOUND_DANGER | SOUND_MOVE_AWAY, GetAbsOrigin(), 2048, 15.0 ); // According to my calculations, this is like a nuke going off.
