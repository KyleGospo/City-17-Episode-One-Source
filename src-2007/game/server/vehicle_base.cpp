...
#ifdef C17_HAPTICS
	// Haptics start our rate timer
#if HAP_VEHICLE_TIME_BASE
	m_flNextHapticsReport = gpGlobals->curtime;
#else
	m_iHapticsSample = 0;
#endif
#endif
...
#ifdef C17_HAPTICS
		// Haptics tell the client who is trying to drive that they've entered this type of vehicle.
		pPlayer->HapticsVehicleStart(m_nVehicleType);
#endif
	}
	else
	{
...
#ifdef C17_HAPTICS
ConVar hap_sv_vehicle_rate_wheel("hap_vehicle_jeep_samples","4",FCVAR_ARCHIVE,"Rate (in milliseconds) vehicle related reports are sent to clients)");
ConVar hap_sv_vehicle_rate_boat("hap_vehicle_airboat_samples","30",FCVAR_ARCHIVE,"Rate (in milliseconds) vehicle related reports are sent to clients)");
ConVar hap_sv_vehicle_smooth("hap_sv_vehicle_smooth","0",FCVAR_ARCHIVE);
// Haptics Updates the driver clientside of the vehicles velocity
void CPropVehicleDriveable::VPhysicsUpdate(IPhysicsObject *pPhysics )
{
	BaseClass::VPhysicsUpdate(pPhysics);
	if( GetDriver() && GetDriver()->IsPlayer() && 
#if HAP_VEHICLE_TIME_BASE
		(gpGlobals->curtime>m_flNextHapticsReport) 
#else
		( m_iHapticsSample >= 
		( (m_nVehicleType & VEHICLE_TYPE_CAR_WHEELS) ? hap_sv_vehicle_rate_wheel.GetInt() : hap_sv_vehicle_rate_boat.GetInt() )
		)		
#endif
		)
	{
		...
		if(hap_sv_vehicle_smooth.GetInt() == 1)
			VPhysicsGetObject()->WorldToLocalVector( &localVelocity,
				BaseClass::GetSmoothedVelocity());
		else
			VPhysicsGetObject()->WorldToLocalVector( &localVelocity,
				velocity);

		pPlayer->HapticsVelocityUpdate(localVelocity);
#if HAP_VEHICLE_TIME_BASE
		float Interval = hap_sv_vehicle_rate_wheel.GetFloat()/1000;
		if(m_nVehicleType & VEHICLE_TYPE_AIRBOAT_RAYCAST)
		{
			Interval = hap_sv_vehicle_rate_boat.GetFloat()/1000;
		}

		// set our timer point up again and make up for small delays in time
		m_flNextHapticsReport = ( gpGlobals->curtime - fmod( ( gpGlobals->curtime - m_flNextHapticsReport), Interval ) ) +Interval;
#else
		m_iHapticsSample = 0;
#endif
	}
